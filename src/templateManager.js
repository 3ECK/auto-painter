 ...
 *     "1 $Z": {
 *       "name": "My Template",
 *       "URL": "https://github.com/3ECK/auto-painter/blob/main/dist/assets/Favicon.png",
 *       "URLType": "template",
 *       "enabled": false,
 *       "tiles": {
...
export default class TemplateManager {
  ...
  constructor(name, version, overlay) {
    ...
    this.wrongPixelsMap = new Map(); // Tracks specific wrong pixels per tile { tileKey: [ {x,y,color,priority} ] }
  ...
  async drawTemplateOnTile(tileBlob, tileCoords) {
    ...
    // Store localized list of wrong pixels for this refresh
    const tileWrongPixels = [];
        ...
        try {
          ...
          for (let y = 0; y < tempHeight; y++) {
            for (let x = 0; x < tempWidth; x++) {
              ...
              // Handle template transparent pixel (alpha < 64)
              if (templatePixelCenterAlpha < 64) {
                // Check if this is a "Deface" marker (Alpha ~32) or just transparent (Alpha 0)
                // In Template.js, deface pixels are set to alpha 32.
                if (templatePixelCenterAlpha > 10 && templatePixelCenterAlpha < 50) {
                  requiredCount++;

                  try {
                    const activeTemplate = this.templatesArray?.[0];
                    const tileIdx = (gy * drawSize + gx) * 4;
                    // const pr = tilePixels[tileIdx];    // unused
                    // const pg = tilePixels[tileIdx + 1]; // unused
                    // const pb = tilePixels[tileIdx + 2]; // unused
                    const pa = tilePixels[tileIdx + 3];

                    // If board has pixel (alpha >= 64), it's WRONG because we want ID 0 (Transparent)
                    // Note: ID 0 implies erasing.
                    if (pa >= 64) {
                      wrongCount++;

                      // Add to candidate list for "erasing"
                      const absX = Number(tileCoords.split(',')[0]) * this.tileSize + (gx / this.drawMult);
                      const absY = Number(tileCoords.split(',')[1]) * this.tileSize + (gy / this.drawMult);

                      tileWrongPixels.push({
                        x: Math.floor(absX),
                        y: Math.floor(absY),
                        // Use the DEFACE KEY so rgbToMeta finds ID 0
                        color: [222, 250, 206],
                        priority: 1
                      });
                    }
              ...
              if (realPixelCenterAlpha < 64) {
                ...
                tileWrongPixels.push({
                  x: Math.floor(absX),
                  y: Math.floor(absY),
                  color: [templatePixelCenterRed, templatePixelCenterGreen, templatePixelCenterBlue],
                  priority: 1
                });
    ...
    if (templateCount > 0) {
      ...
      // Update wrong pixels map for this tile
      console.log(`[BM DEBUG] Setting wrongPixelsMap for ${tileKey}: ${tileWrongPixels.length} pixels`);
      this.wrongPixelsMap.set(tileKey, tileWrongPixels);
  ...
  /** Retrieves all currently detected wrong pixels from all cached tiles.
   * @returns {Array<{x:number, y:number, color:number[]}>}
   */
  getAllWrongPixels() {
    const all = [];
    console.log(`[BM DEBUG] getAllWrongPixels: Map size = ${this.wrongPixelsMap.size}`);
    for (const [key, list] of this.wrongPixelsMap.entries()) {
      if (list && list.length > 0) {
        // console.log(`[BM DEBUG] Tile ${key} has ${list.length} wrong pixels`);
        all.push(...list);
      } else {
        console.log(`[BM DEBUG] Tile ${key} has 0 wrong pixels`);
      }
    }
    console.log(`[BM DEBUG] getAllWrongPixels: Returning ${all.length} candidates`);
    return all;
  }
